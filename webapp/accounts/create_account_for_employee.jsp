...
<body>
	...
	<table width="100%" border="0" cellspacing="0">
		...
		<tr>
			<td id="navigation"><a href="../menu" target="_top" class="links">Main Menu</a> &gt; <a href="/accounts?account-action=view-accounts" target="_top" class="links">Account List</a> &gt;<em><strong>Account
						for Existing Employee</strong>
			</em>
			</td>
			<td>&nbsp;</td>
		</tr>
 ...
 %>
			...
			if (passwordOK != null && passwordOK.equalsIgnoreCase("false")) {%> <%}%>>Password:</td>
							...
							<tr>
								...
								<td><select name="accountType" class="required_menulist" id="accountType">
										<%
											for (int j = 0; j < accountTypes.length; j++) {
										%>
										<option <%if (accountType != null) {
					if (accountTypes[j].toString().equals(accountType)) {%> selected="selected"
											<%}
				} else {
					if (accountTypes[j].equals(accountTypes[j].SPSO_PERSONNEL)) {%> selected="selected" <%}
				}%>
							...
							<tr>
								...
								<td><input name="createBtn" type="submit" class="button" id="createBtn" value="CREATE" />
								</td>
							</tr>
						</table></td>
				</tr>
